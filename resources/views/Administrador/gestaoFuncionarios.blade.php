    ...
    <div>
        <header class="bg-white shadow-sm border-b border-gray-100 px-8 py-3">
            <div class="max-w-7xl mx-auto flex justify-between items-center">
                <div class="flex items-center gap-2">
                    <div
                        class="bg-[#0D749B] text-white p-2 rounded-full w-9 h-9 flex items-center justify-center font-bold text-lg">
        ...
        <main class="max-w-7xl mx-auto w-full p-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                ...
                <div class="bg-white rounded-3xl p-8 shadow-sm min-h-[500px]">
                    ...
                    <div class="space-y-6">
                        @foreach ($funcionarios as $funcionario)
                            <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                                <div class="flex items-center gap-4">
                                    <div
                                        class="w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center font-bold text-lg text-gray-700 uppercase">
                                    ...
                                    <div>
                                        ...
                                        {{-- Exibe o CPF --}}
                                        <p class="text-xs text-gray-500 mt-0.5">CPF:
                                            {{ $funcionario->cpf ?? 'Não informado' }}</p>
                                        {{-- Exibe o contato correto puxado da tabela --}}
                                        <p class="text-xs text-[#09738A] mt-0.5">
                                            <i class="fa-solid fa-phone text-[10px]"></i>
                                            {{ $funcionario->contato ?? 'Sem contato' }}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-3">
                                    <span
                                        class="bg-[#B9EBF2] text-[#09738A] text-xs font-semibold px-4 py-1 rounded-md capitalize mr-2">
                                        {{ $funcionario->perfil ?? 'Funcionário' }}
                                    </span>

                                    {{-- Botão de Editar (Redireciona para a rota de edição) --}}
                                    <a href="/gestaoFuncionarios/{{ $funcionario->id }}/editar"
                                        class="text-[#00C1DE] hover:text-[#00A9C4] transition">
                                        <i class="fa-regular fa-pen-to-square text-lg"></i>
                                    </a>

                                    {{-- Formulário de Exclusão (Invisível visualmente, ativa no clique) --}}
                                    <form action="/gestaoFuncionarios/{{ $funcionario->id }}" method="POST"
                                        onsubmit="return confirm('Tem certeza de que deseja excluir este funcionário?')\" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="text-[#E05353] hover:text-[#C94242] transition align-middle">
                        ...
                        @if ($funcionarios->isEmpty())
                            <p class="text-gray-400 text-sm text-center py-8">Nenhum funcionário cadastrado no momento.
                            </p>
                        @endif
                    </div>
                </div>
                ...
                <div>
                    <div x-show="!mostrarFormulario" @click="mostrarFormulario = true"
                        ...
                        <form action="/gestaoFuncionarios" method="POST" class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="nome" required
                                    class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:outline-none focus:border-[#00C1DE] text-sm">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="perfil" required
                                    class="w-full px-4 py-2.5 bg-[#B9EBF2]/40 border border-transparent rounded-xl focus:outline-none text-sm text-[#09738A] font-semibold cursor-pointer">
                            ...
                            <div class="pt-2">
                                <button type="submit"
                                    class="w-full bg-[#00C1DE] text-white py-2.5 rounded-full font-bold text-sm hover:bg-[#00A9C4] transition shadow-sm">
